import { mat4, Node, Enum, _decorator, ScrollView, Prefab, Vec2, Size, NodePool, instantiate, Vec3, UITransform, Rect, clamp } from "cc";
...
export class DynamicScrollView extends ScrollView {
    @property({
        type: Size,
        tooltip: DEV && "默认item尺寸",
    })
    public defaultItemSize:Size;

    @property({
        type: Prefab,
        tooltip: DEV && "item的模板",
    })
    public itemTemplate : Prefab;
    ...
    protected layoutType : ItemLayoutType;

    // 只保存4个临界index
    protected criticalItemIndex : number[] = new Array<number>(4);

    // callbacks for items
    protected updateFunc : (index: number, item:UITransform) => void;
    protected itemSizeFunc: (index:number) => Size;
    protected itemCountFunc: () => number;
    protected itemGetFunc: (index:number) => UITransform;
    protected itemRecycleFunc: (item:UITransform) => void;

    private managedItems: Array<ScrollItemWithRect> = new Array<ScrollItemWithRect>();

    private refRect : Rect;

    // resource management
    private itemPool : NodePool = null;
    ...
    private viewWorldConers: Array<Vec3> = new Array<Vec3>(4);
    private rectCorners: Array<Vec3> = new Array<Vec3>(2);

    public SetUpdateFunc(func : (index: number, item: UITransform) => void) : void {
        this.updateFunc = func;
    }

    public SetItemSizeFunc(func :(index: number) => Size) : void {
        this.itemSizeFunc = func;
    }

    public SetItemCountFunc(func :() => number) : void {
    ...
    public override onLoad(): void{
        ...
        super.onLoad?.();
        this.layoutType = ItemLayoutType.Vertical;
    }

    public SetItemGetAndRecycleFunc(getFunc : (index: number) => UITransform, recycleFunc :(item: UITransform) => void) :void {
    ...
    private GetCriticalItem(type:number) : UITransform {
    ...
    private CheckAndHideItem(criticalItemType:number):boolean {
        const item :UITransform= this.GetCriticalItem(criticalItemType);
    ...
    private ShouldItemSeenAtIndex(index:number):boolean {
        ...
        const contentBase : Vec2 =new Vec2(this.content.position.x, this.content.position.y);
        const origin : Vec2 = this.refRect.origin.subtract(contentBase);
